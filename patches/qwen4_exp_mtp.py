...
class Qwen4ExpForCausalLMMTP(Qwen3_5ForCausalLMMTP):
    ...
    @staticmethod
    def _linear_out(mod, x: torch.Tensor) -> torch.Tensor:
        y = mod(x)
    ...
    def _init_linear_projections(
        ...
        # EXL3 packs store mtp.fc_embedding / mtp.fc_hidden as trellis.
        # Plain nn.Linear never binds those keys (accept rate stays 0).
    ...
    def _fuse_residual_linear_shared(
        ...
        input_embeds = self._linear_out(
            self.fc_embedding, self.pre_fc_norm_embedding(input_embeds)
        )
        ...
        encoder_inputs = self._linear_out(self.fc_hidden, decoder_view)
